...
(function (global) {
  ...
  function _cssVar(name, fb){ try{ var v=getComputedStyle(document.documentElement).getPropertyValue(name).replace(/^\s+|\s+$/g,""); return v||fb; }catch(e){ return fb; } }
  ...
  RadioGlobe.prototype._draw = function (time) {
    ...
    bg.addColorStop(0, "rgba(" + RGB + ",0.10)");
    ...
    glow.addColorStop(0.8, "rgba(" + RGB + ",0.05)");
    ...
    sphere.addColorStop(0, "rgba(" + RGB + ",0.14)");
    sphere.addColorStop(0.55, "rgba(0,0,0,0.34)");
    sphere.addColorStop(1, "rgba(0,0,0,0.66)");
    ...
    for (i = 0; i < markers.length; i++) {
      ...
      ctx.fillStyle = "rgba(" + RGB + "," + (0.35 + mz2 * 0.45).toFixed(3) + ")";
      ctx.beginPath();
    ...
    if (activePt) {
      ...
      /* pierścień pulsu */
      ctx.strokeStyle = "rgba(" + RGB + "," + (0.6 - pulse * 0.35).toFixed(3) + ")";
      ctx.lineWidth = 1.6;
      ctx.beginPath();
      ...
      ag.addColorStop(0, "rgba(" + RGB + ",0.55)");
      ...
      if (this.activeLabel) {
        ...
        ctx.fillStyle = "rgba(0,10,4,0.74)";
