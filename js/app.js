...
var STATION_CACHE_KEY = "radioantomatee-stations-v2";
...
/* — kraje: nazwy po polsku + flagi emoji — */
...
function setNowPlaying(st){
  ...
  var meta = ccFlag(st.country) + " " + cn + " · " + st.genre +
...
function renderFilterOptions(){
  ...
  var ccList = Object.keys(ccMap).map(function(cc){
    return { cc: cc, label: ccFlag(cc) + " " + countryName(cc) };
  });
...
function sortView(list){
  ...
  list.sort(function(a,b){
    ...
    if (mode==="country") {
      ...
      if (an2 !== bn2) return an2.localeCompare(bn2, "pl");
    }
  ...
  return '<div class="' + rowClass(i, st) + '" data-row="1" data-index="' + i + '">' +
    ...
    '<div class="rowInfo">' +
      ...
      '<div class="stationSub">' +
        ...
        '<span class="rowFlag">' + ccFlag(st.country) + '</span> ' +
